...
export const useCases: UseCase[] = [
  ...
  },
  {
    ...
    how: [
      { icon: 'ph-chart-bar', title: '사용 가시화', desc: '설비와 구역, 시간대를 함께 놓고 전력을 봅니다. 어느 설비가 언제 얼마나 쓰는지가 한 화면에 모이면, 막연하던 소비가 눈에 잡히기 시작합니다.' },
      { icon: 'ph-chart-line', title: '피크 관리', desc: '여러 설비가 동시에 돌아 피크를 만드는 순간을 찾습니다. 가동 시점을 조금씩 어긋나게 하거나 부하를 나누면, 계약전력과 피크 요금이 함께 내려갑니다.' },
      { icon: 'ph-warning-circle', title: '이상 사용 감지', desc: '평소 패턴에서 벗어난 소비를 잡아냅니다. 꺼져 있어야 할 시간에 도는 설비나 슬그머니 늘어난 대기 전력이 여기서 드러납니다.' },
  ...
  },
  {
    ...
    how: [
      { icon: 'ph-wave-sine', title: '품질 지표 감시', desc: '전압과 주파수, 고조파, 불평형 같은 지표를 실시간으로 지켜봅니다. 기준을 벗어나는 순간을 놓치지 않습니다.' },
      { icon: 'ph-clipboard-text', title: '이벤트 자동 기록', desc: '순간 전압 강하(sag)나 상승(swell), 짧은 정전처럼 스쳐 지나가는 사건을 자동으로 기록해 둡니다. 나중에 되짚어 볼 근거가 남습니다.' },
      { icon: 'ph-link', title: '원인 연결', desc: '품질 이상이 언제 어느 설비 근처에서 있었는지를 가동·고장 이력과 겹쳐 봅니다. 멈춤이나 불량과 시점이 맞는지 확인하면, 막연한 의심이 근거로 바뀝니다.' },
  ...
  },
  {
    ...
    how: [
      { icon: 'ph-factory', title: '통합 계량', desc: '전기·가스·스팀·용수와 설비별 소비를 한 구조로 모아, 공장 전체의 에너지 흐름을 하나의 화면에서 봅니다.' },
      { icon: 'ph-chart-donut', title: '원단위 분석', desc: '생산량 대비 에너지 사용(원단위)을 비교해, 같은 일을 하는데 더 많이 쓰는 구간과 시점을 찾아냅니다.' },
      { icon: 'ph-chart-line', title: '피크 · 부하 관리', desc: '여러 설비가 몰려 피크를 만드는 시점을 파악해 부하를 분산하고, 계약전력과 요금 피크를 낮춥니다.' },
  ...
  },
  {
    ...
    how: [
      { icon: 'ph-flow-arrow', title: '공정-품질 연결', desc: '공정 조건과 검사 결과를 이어, 어떤 조건이 불량과 상관 있는지 봅니다.' },
      { icon: 'ph-waveform', title: '전조 감지', desc: '온도·압력·원료 특성의 미세 변화에서 불량 전조를 읽습니다.' },
      { icon: 'ph-bell-ringing', title: '사전 경고', desc: '위험 구간에 들어서면 근거와 함께 조치를 안내합니다.' },
    ],
